...
plot(ages)    #if you plot on single with one value it will take y axis automatically
...
data()
airquality = datasets::airquality  # to bring  pre defined data sets 
#can change            #cant change 
...
boxplot.stats(airquality$Wind)$out #outlier values     # to get outliers from the table 
...
help(par)  #partition = par 
par(mfrow=c(3,3),mar=c(1,2,1,2),  las=0, bty="o")  #mfrowws = numbers of rows and columns
...
max(airquality$Solar.R,na.rm=T)   # to  get individual values oof mean separately by removing the values of NA
#we ge to remove the na values 
mean(airquality$Solar.R,na.rm=T)

#in same way we remove na values by removing the NA values ,,, use NA.rm values to reset 
